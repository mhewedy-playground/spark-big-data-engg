...
public class ProjectEUSalesDataProcessor {
    public static void main(String[] args) {
        ...
        try {
            ...
            JavaSparkContext.toSparkContext(streamingContext.sparkContext()).register(salesMap);

            // Setup a DB Connection to save summary
            Class.forName("com.mysql.jdbc.Driver").newInstance();
            Connection execConn = DriverManager
                    .getConnection("jdbc:mysql://localhost:3306/exec_reports?user=cloudera&password=cloudera");

            // Setup a processing map function that only returns the payload.
            JavaDStream<String> retval = stream.map(record -> {
                        try {
            ...
            retval.foreachRDD(__ -> {
                ...
                for (String salesDate : salesMap.value().keySet()) {
                    ...
                    PreparedStatement stmt = execConn.prepareStatement(updateSql);
